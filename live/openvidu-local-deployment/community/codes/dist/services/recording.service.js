...
import { OpenViduCallError, errorRecordingAlreadyStarted, errorRecordingNotFound, errorRecordingNotStopped, internalError } from '../models/error.model.js';
...
export class RecordingService {
    ...
    async deleteRecording(egressId, isRequestedByAdmin) {
        try {
            ...
            const recordingInfo = (await this.s3Service.getObjectAsJson(metadataPath));
            ...
            if (!recordingPath)
                throw internalError(`Error extracting path from recording ${egressId}`);
     ...
     */
    async getAllRecordings(continuationToken) {
        try {
            ...
            allEgress.Contents?.forEach((item) => {
                if (item?.Key?.includes('.json')) {
                    promises.push(this.s3Service.getObjectAsJson(item.Key));
                }
            });
     ...
     */
    async getAllRecordingsByRoom(roomName, roomId) {
        try {
    ...
    async getRecording(egressId) {
        try {
            ...
            const recording = await this.s3Service.getObjectAsJson(metadataObject.Contents[0].Key);
            return recording;
    ...
    async getRecordingAsStream(recordingId, range) {
        try {
            ...
            if (!recordingPath)
                throw new Error(`Error extracting path from recording ${recordingId}`);
     ...
     */
    generateFileOutputFromRequest(recordingId, filePathOrFileName) {
        let filepath;
        if (!filePathOrFileName)
            filePathOrFileName = recordingId;
