...
export class RoomService {
     ...
     */
    async isRoomCreatedByMe(roomOrRoomName) {
        try {
            let room;
     ...
     */
    async sendSignal(roomName, rawData, options) {
        try {
            if (this.roomClient) {
                ...
                this.logger.verbose(`Sending signal "${options.topic}" to ${options.destinationIdentities
                    ? `participant(s) ${options.destinationIdentities}`
                    : 'all participants'} in room "${roomName}".`);
