...
export class WebhookService {
    ...
    recordingService = RecordingService.getInstance();
    broadcastingService = BroadcastingService.getInstance();
     ...
     */
    async getEventFromWebhook(body, auth) {
        return await this.webhookReceiver.receive(body, auth);
    }
    async handleEgressUpdated(egressInfo) {
        try {
            const isRecording = RecordingHelper.isRecordingEgress(egressInfo);
            const isBroadcasting = BroadcastingHelper.isBroadcastingEgress(egressInfo);
            if (!isRecording && !isBroadcasting)
                return;
     ...
     */
    async handleParticipantJoined(room, participant) {
        try {
    ...
    async sendStatusSignal(roomName, roomId, participantSid) {
